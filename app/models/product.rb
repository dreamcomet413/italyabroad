class Product < ActiveRecord::Base
  ...
  attr_accessor :price, :quantity , :is_vertical
 ...
 # self.find(:all, :include => [:categories, :grapes], :conditions => conditions ).group_by(&:color).each do |t,  product|
  ...
  def out_of_stock?
	...
	def average_rating
	  reviews = self.shared_reviews.find(:all,:conditions=>['publish = true  && score is not null '])
		return 0 if shared_reviews.empty?

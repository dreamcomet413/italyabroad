  ...
  def index
     if params[:search]
      date=Date.new(params[:order_date]['(1i)'].to_i,params[:order_date]['(2i)'].to_i, params[:order_date]['(3i)'].to_i)
       @orders = Order.where(['status_order_id LIKE ? AND (users.name  like ? OR users.login like ? or users.surname LIKE ? ) AND DATE(orders.created_at) <= ?',"%#{params[:status]}%","%#{params[:search_text]}%","%#{params[:search_text]}%","%#{params[:search_text]}%","#{date}"]).includes([:user, :status_order, :gift_option, :payment_method, :order_items]).order("orders.created_at DESC").paginate(:page => params[:page], :per_page => 20)
  else
    @orders = Order.where("").includes([:user, :status_order, :gift_option, :payment_method, :order_items]).order("orders.created_at DESC").paginate(:page => params[:page], :per_page => 20)
end
 ...
 #    make_and_send_pdf("/admin/orders/#{@order.id}/print_invoice", "Italyabroad_Invoice_#{@order.id}.pdf")
  ...
  def delivery_details
    ...
    if params[:submit] == "Deliver"
      ...
      Notifier.order_details_after_shipping(@order).deliver

...
def index
  ...
  #-------------
    ...
    #@type = Type.find(:first,:conditions=>['upper(name) = ?',params[:category].upcase])
    #  if @searched_text.length > 3
    @users = Grape.find(:all, :conditions => ["name LIKE ? ", "%#{params[:text]}%"]).paginate(:page => params[:page], :per_page => 10)
    #  end
  ...
  @wine_events = Product.find(:all, :include => [:categories] ,:conditions => ['upper(categories.name) LIKE ? AND products.name LIKE ? AND products.active = ?  AND DATE(date) >= ?','EVENTS',"%#{params[:text]}%",true,Date.today]).paginate(:page => params[:page], :per_page => 10)
  end

  def find_grapes
  @grapes = Grape.find(:all, :conditions => ['grapes.name LIKE ? ',"%#{params[:text]}%"]).paginate(:page => params[:page], :per_page => 10)
  end

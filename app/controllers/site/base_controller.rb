class Site::BaseController < ApplicationController
  ...
  def index
    ...
    @recommended_wines = Product.where("categories.id = ? AND products.raccomanded = ?", wine_categories.last.try(:id), true).includes([:categories]).order("created_at ASC").limit(4)

    food_categories = Category.find_by_sql("select * from categories where parent_id is null and name='Food'")
    @food_counter = Product.where("categories.id = ? AND products.raccomanded = ?", food_categories.first.try(:id), true).includes([:categories]).order("created_at ASC").limit(4)
    ...
    @other_drinks = Product.where("categories.id = ? AND products.raccomanded = ? ", other_categories.last.try(:id), true).includes([:categories]).order("created_at ASC").limit(4)

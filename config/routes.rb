...
#  map.connect "products.xml",:controller=>'site/products',:action=>'index'
 ...
 map.enable_disable_chat 'enable_disable_chat', :controller => 'admin/base', :action => 'enable_disable_chat'
  ...
  map.resources :search, :controller => 'site/search', :only => [:index],:collection=>[:find_users,:find_wines,:find_foods,:find_recipes,:find_producers,:find_wine_events, :find_grapes,:find_hampers]
...
map.eval_xml_g_comptible 'admin/xml/eval_xml_g_comptible', :controller => 'admin/xml', :action => 'eval_xml_g_comptible'
  ...
  map.namespace :admin do |admin|
    ...
    admin.resources :orders, :only => [:index, :show, :destroy], :member => {:print_tasting => :get, :print_invoice => :get, :print_picking_list => :get,:delivery_details=>:get,:cancel_delivery_charge=>:get}
